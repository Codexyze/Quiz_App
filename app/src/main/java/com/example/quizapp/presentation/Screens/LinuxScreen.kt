...
import com.example.quizapp.presentation.AllViewmodel.LinuxViewModel

@Composable
fun GetLinuxQuestionScreen(viewModel: LinuxViewModel= hiltViewModel()) {
    val state = viewModel.getLinuxResponseState.collectAsState()
    var score by rememberSaveable { mutableStateOf(0) } // Score counter

    LaunchedEffect(Unit) {
        viewModel.getLinuxQuestions()
    }
    ...
    Column(
        ...
        Text(
            ...
            modifier = Modifier.padding(bottom = 16.dp)
                .testTag(TestTags.LINUX_SCORE)
        )

        when (val currentState = state.value) {
            is LinuxResponseState.Idle -> {
                // Do nothing, waiting for data
            }
            is LinuxResponseState.Loading -> {
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    CircularProgressIndicator()
                }
            }
            is LinuxResponseState.Error -> {
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    Text("Error loading data", color = Color.Red, fontSize = 18.sp, fontWeight = FontWeight.Bold)
                }
            }
            is LinuxResponseState.Success -> {
                LazyColumn(
                    modifier = Modifier.fillMaxSize(),
                    verticalArrangement = Arrangement.spacedBy(16.dp)
                ) {
                    itemsIndexed (currentState.data) { index,questionItem ->
                        ...
                        Card(
                            ...
                            Column(
                                ...
                                Text(
                                    ...
                                    fontWeight = FontWeight.Bold,
                                    color = Color.Black,
                                    modifier = if (index == 0) Modifier.testTag(TestTags.LINUX_SCROLL)else Modifier
                                )
