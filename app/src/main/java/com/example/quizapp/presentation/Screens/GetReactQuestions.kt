...
import com.example.quizapp.presentation.AllViewmodel.ReactQuestionsViewModel

//viewModel: ViewModel = hiltViewModel()
@Composable
fun GetReactQuestionScreen(viewModel: ReactQuestionsViewModel = hiltViewModel()) {
    ...
    Column(
        ...
        Text(
            ...
            modifier = Modifier.padding(bottom = 16.dp)
                .testTag(TestTags.REACT_SCORE)
        )

        when (val currentState = state.value) {
            is ReactResponseState.Idle -> {
                // Do nothing, waiting for data
            }
            is ReactResponseState.Loading -> {
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    CircularProgressIndicator()
                }
            }
            is ReactResponseState.Error -> {
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    Text("Error loading data", color = Color.Red, fontSize = 18.sp, fontWeight = FontWeight.Bold)
                }
            }
            is ReactResponseState.Success -> {
                LazyColumn(
                        ...
                        Card(
                            ...
                            Column(
                                ...
                                Text(
                                    ...
                                    fontWeight = FontWeight.Bold,
                                    color = Color.Black,
                                    modifier = if (index == 0) Modifier.testTag(TestTags.REACT_SCROLL)else Modifier
                                )
